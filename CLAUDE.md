...
**Browser-based flash erase (recovery tool):** If the device is stuck in a boot loop and `scripts/flash.sh` can't connect, use the Espressif web tool at https://espressif.github.io/esptool-js/ to erase flash via WebUSB directly from the browser. This bypasses macOS USB driver issues entirely. Use "Erase Flash" to wipe the chip, then run `scripts/flash.sh` to reflash.
