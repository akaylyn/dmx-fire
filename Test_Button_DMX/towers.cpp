...
// All towers share address 1 so fire triggers all fixtures simultaneously.
...
void towerWrite(uint8_t index, const TowerState& state) {
  ...
  dmxShadowWrite(state.r,    base + 1);  // CH1: Red
  dmxShadowWrite(state.g,    base + 2);  // CH2: Green
  dmxShadowWrite(state.b,    base + 3);  // CH3: Blue
  dmxShadowWrite(state.wDim, base + 4);  // CH4: White
  ...
  dmxShadowWrite(state.r,         s + 5);  // CH5: Red
  dmxShadowWrite(state.g,         s + 6);  // CH6: Green
  dmxShadowWrite(state.b,         s + 7);  // CH7: Blue
  ...
  dmxShadowWrite(state.wDim,      s + 11); // CH11: white dimmer
}
