...
> the tower uplights now run in **4-channel mode** (R/G/B/W at base+5 … base+8) rather than
> 11-channel — see [spec-uplight-4ch-mode.md](spec-uplight-4ch-mode.md). Confluence channel
> numbers below have been corrected; the 11-channel uplight references are left as historical
...
- **Simon towers (×4)**: RGBW LED decoder + 11ch strobe per tower. **CH4 of the decoder triggers a flame effect** — each tower has fire capability. CH1–3 are RGB visuals.
- **Confluence**: A single art-piece tower with a **real propane solenoid** on DMX CH1 — a 3-channel decoder at A001 with the solenoid wired to its first output. CH2–3 are unwired decoder outputs. Sits first in the DMX chain.

The lockout is a **safety mechanism**: prevents solenoids from being held open continuously. The button triggers fire on **all connected towers** (Confluence CH1 + Simon tower decoder CH4s) along with light patterns.
...
Confluence sits at the bottom of the universe on a 3-channel decoder at A001, but only CH1 matters — it opens the propane solenoid. CH2–3 are unwired decoder outputs and CH4 is unclaimed by any fixture; all three are written as 0.

### New universe layout

```
Confluence:  ch  1– 4  (only ch 1 = solenoid; ch 2–4 written as 0)
...
struct ConfluenceConfig {
  ...
  uint8_t fireLevel;   // 0=off, 255=full open (written to ch 1)
};
extern ConfluenceConfig confluenceConfig;

void confluenceSetup();
void confluenceWrite(uint8_t fireLevel);  // writes fireLevel,0,0,0 to ch 1–4
```

- `confluenceSetup()`: defaults to connected=true, fireLevel=255
- `confluenceWrite(level)`: writes `dmxDevice.writeByte(level, 1)` and `dmxDevice.writeByte(0, 2..4)`

### Simon tower flame support

Each `TowerConfig` gains a `flameLevel` field (written to the decoder's CH4, the valve output):

```cpp
struct TowerConfig {
  // existing fields ...
  uint8_t flameLevel;  // 0=off, 255=full open; written to decoder ch 4
};
```

`towerWrite()` already writes all 4 decoder channels; `flameLevel` populates the fourth (valve) byte. The web UI gets a per-tower flame level slider (disabled if `connected=false`).
...
- `IDLE` — waiting; towers show idle palettes; all valve channels = 0
- `FIRE_ACTIVE` — **Confluence CH1 = 255 AND each connected Simon tower decoder CH4 = 255**; towers show fire palette; countdown ticking
- `END_CUE` — all valve channels → 0; towers play end-of-fire cue pattern (~1 s)
...
- **Flame palettes for Confluence-only events** (no Simon towers) — pre-built colour sequences mimicking fire, for any LED-capable fixture later added to the universe. Note that the Confluence decoder's own ch 1–3 are not available for this: ch 1 is the solenoid and ch 2–3 are its unwired outputs.
...
| File | Change |
|------|--------|
| `confluence.h` / `confluence.cpp` | New — solenoid config + DMX write (ch 1–4, only ch 1 active) |
...
1. Compile clean
2. Confluence CH1 goes to 255 on button press, returns to 0 on release / timer expiry
3. Simon tower decoder CH4s also go to their configured flameLevel on button press
